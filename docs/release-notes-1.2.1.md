...
**Release candidate: final packaging and publication checks are in progress.**
...
The integrated Apple Silicon Release build and all 221 tests passed, including
media recovery, native-layout, audio-DSP, image-copy and runtime regressions.
Real PDF/DOCX conversions also passed. Final installer checks are pending.
Authenticated Spotify playback, live EQ adjustment, bypass and pause/resume
recovery passed in the installed candidate. Fresh capture-permission grant/denial,
output-device switching and execution on a recipient's macOS 15 Mac remain untested. Process EQ handles local Mac
audio; it does not equalize Spotify Connect playback on another device. Chrome
EQ can affect other audible Chrome tabs.

Final signing/notarization, GitHub downloads and Homebrew upgrade/reinstall
results will be recorded in the [verification matrix](release-verification-matrix.md)
before this candidate is described as published. Existing 1.2.0 download files
remain unchanged.
