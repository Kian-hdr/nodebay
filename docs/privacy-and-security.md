...
The optional Browser Media Bridge runs locally. Its Chrome extension initially has access to `www.youtube.com` and `music.youtube.com`; a user can explicitly grant optional HTTP and HTTPS site access from the extension popup to control other media tabs. It sends Nodebay a tab identifier, visible media title and artist, playback timing/state, volume, supported-control flags, and whether that tab's equalizer capture is enabled. Page URLs are forwarded only for YouTube tabs used by Nodebay's download action. Chrome native messaging hands this to a bundled host, which connects only to Nodebay's loopback listener. Nodebay does not operate a relay, analytics service, or media-metadata server. Equalizer audio remains inside Chrome's local Web Audio graph and is never sent to Nodebay or a server. Audio capture requires a separate **Enable EQ for This Tab** action for each YouTube tab and is stopped on bypass, source change, tab close, or bridge disconnect.
