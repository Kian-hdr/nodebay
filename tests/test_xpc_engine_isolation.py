...
class XPCEngineIsolationTests(unittest.TestCase):
    def test_helper_is_allowlisted_and_never_invokes_a_shell(self):
        ...
        self.assertIn('case "stl-repair"', HELPER)
        self.assertIn('cancelledProcessIDs.remove(jobID)', HELPER)
        self.assertIn('try? stdout.fileHandleForWriting.close()', HELPER)
        self.assertIn('kill(process.processIdentifier, SIGKILL)', HELPER)
