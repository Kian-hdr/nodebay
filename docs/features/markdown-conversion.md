...
Compatible non-Markdown documents show **Convert to MD**. Existing `.md` and `.markdown` files never show the action. Each conversion runs outside the main UI through bundled, unmodified Microsoft MarkItDown 0.1.7 and writes an atomic, collision-safe Markdown copy in Nodebay-managed storage before adding it to the shelf.
