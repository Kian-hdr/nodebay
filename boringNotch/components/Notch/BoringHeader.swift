...
struct BoringHeader: View {
    ...
    @ObservedObject private var longhaul = LonghaulCompanionClient.shared
    var body: some View {
        HStack(spacing: 0) {
            HStack {
                if (!tvm.isEmpty || coordinator.alwaysShowTabs || chat.enabled) && Defaults[.boringShelf] {
                    TabSelectionView()
            ...
            HStack(spacing: 4) {
                if vm.notchState == .open {
                    if isOSDType(coordinator.sneakPeekState(for: vm.screenUUID).type) && coordinator.shouldShowSneakPeek(on: vm.screenUUID) && Defaults[.showOpenNotchOSD] {
                        ...
                        if longhaul.showsNotchControl {
                            LonghaulNotchButton()
                        }
                        if coordinator.currentView == .chat {
                            QuickChatHeaderActions()
                        }
